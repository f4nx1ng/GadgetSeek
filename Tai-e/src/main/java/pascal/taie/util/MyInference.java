...
import pascal.taie.analysis.pta.plugin.TestHandler;
                    ...
                    if(assignStmt instanceof StoreArray sa){
                        ...
                        if (lf.getFieldAccess() instanceof InstanceFieldAccess instanceFieldAccess && instanceFieldAccess.getBase().equals(current.var)){
                            ...
                            if (lvar.equals(var)){
                                ...
                                stack.push(new Node(lvar, lf.getFieldRef().getName(), lf.getFieldRef()));
                            }
...
//                            }
//                        }
                        FieldRef result = instanceInvokeDeal(invokeInstanceExp, current, invoke, var, stack);
                        if (result!=null){
                            return result;
                        }
...
//                        }
    ...
    public static FieldRef instanceInvokeDeal(InvokeInstanceExp invokeInstanceExp, Node current, Invoke invoke, Var var, Stack<Node> stack){
        Var base = invokeInstanceExp.getBase();
        if (base.equals(current.var) || invokeInstanceExp.getArgs().contains(current.var)) {
            Var lvar = invoke.getLValue();
            //todo 先判断是不是null
            if (lvar != null) {
                ...
                if (!base.equals(current.var) && invokeInstanceExp.getArgs().contains(current.var)) {
                    if (base.equals(var)) {
                        ...
                        stack.push(new Node(base, current.fieldname, current.fieldRef));
                        return null;
                    }
                }
            }
    ...
    public static boolean instanceInvokeDealVar(InvokeInstanceExp invokeInstanceExp, Var current, Invoke invoke, Var var, Stack<Var> stack){
        Var base = invokeInstanceExp.getBase();
