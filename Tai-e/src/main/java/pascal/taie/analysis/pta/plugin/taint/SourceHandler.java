...
class SourceHandler extends OnFlyHandler {
    ...
    /**
     * 这个方法用于处理入口方法所在类，可能存在污点的情况
     * class.readObject(ois),ois会产生污点数据，但是class本身可能也存在
     * 具体情况参照日志当中的2024/11/1 问题1
     * 所以我们需要实现如左图所示的传递 ois->{taintobj} ==> this->{taintobj}
     * 当然这其中的indexRef会有问题，
     * */

    private void handleSelfClassTaint(JMethod method, Context context, SourcePoint sourcePoint){
            Var thisVar = method.getIR().getThis();
            Obj taint = manager.makeTaint(sourcePoint, thisVar.getType());
            solver.addVarPointsTo(context, thisVar, taint);
    }
    ...
    private void handleParamSource(CSMethod csMethod) {
        ...
        if (paramSources.containsKey(method)) {
            ...
            paramSources.get(method).forEach(source -> {
                ...
                Obj taint = manager.makeTaint(sourcePoint, source.type());
                handleSelfClassTaint(method, context, sourcePoint);
                switch (indexRef.kind()) {
                    case VAR -> {
