...
class TransferHandler extends OnFlyHandler {
    ...
    private void processTransfer(Context context, Invoke callSite, TaintTransfer transfer) {
        ...
        JMethod jMethod = callSite.getRValue().getMethodRef().resolve();

        if (from.kind() == IndexRef.Kind.VAR) { // Var -> Var/Array/Field
            Kind kind = switch (to.kind()) {
                case VAR -> {
                    Transfer tf = getTransferFunction(transfer.type(), jMethod);
                    solver.addPFGEdge(
...
//        info.transfer().method()
        Transfer tf = getTransferFunction(info.transfer().type(), info.transfer.method());
        switch (info.kind()) {
            case VAR_TO_ARRAY -> {
                baseObjs.objects()
    ...
    /**
     * 在pta当中应用cha的方式对所有的待选类进行解析，从而创建多个隐式的污点对象。
     * 但是该函数仅发生在tansferfunction当中
     * */

    private Transfer getTransferFunction(Type toType, JMethod jMethod) {
...
//                            .map(obj -> manager.makeTaint(manager.getSourcePoint(obj), type, obj))
...
//        return transferFunctions.computeIfAbsent(toType,
//                type -> ((edge, input) -> {
//                    Collection<Type> allTypes = MyUtils.getAllTypes(type, jMethod, null);
//                    PointsToSet newTaints = solver.makePointsToSet();
//                    allTypes.forEach(atype ->{input.objects()
//                            .map(CSObj::getObject)
//                            .filter(manager::isTaint)
//                            .map(obj -> manager.makeTaint(manager.getSourcePoint(obj), type, obj))
//                            .map(taint1->{taint1.setImplicited();
//                                return taint1;
//                            })
//                            .map(taint -> csManager.getCSObj(emptyContext, taint))
//                            .forEach(newTaints::addObject);});
...
//                    }
                    Collection<Type> allTypes = MyUtils.getAllTypes(type, jMethod, null, true);
                    PointsToSet newTaints = solver.makePointsToSet();
                    allTypes.forEach(atype ->{input.objects()
                            .map(CSObj::getObject)
                            .filter(manager::isTaint)
                            .map(obj -> manager.makeTaint(manager.getSourcePoint(obj), atype))
                            .map(taint1->{taint1.setImplicited();
                                return taint1;
                            })
                            .map(taint -> csManager.getCSObj(emptyContext, taint))
