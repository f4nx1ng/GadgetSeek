...
//                        }
        ...
        //want to know how the pts works

        //test section 1, test calledge and taintobj

        //test section 2, test HashMap invokestmt taintobj
//        callGraph.reachableMethods().forEach(csMethod -> {
//            if (csMethod.getMethod().getDeclaringClass().getName().equals("java.util.HashMap") && csMethod.getMethod().getName().equals("readObject")) {
...
//                        if (invokeExp.getMethodRef().resolve().getName().equals("hash")){
//                            System.out.println("after defalut solver");
//                            Set<String> classSet1 = new HashSet<>();
//                            for (Var var1 : invokeExp.getArgs()){
//                                PointsToSet pts2 = csManager.getCSVar(context, var1).getPointsToSet();
//                                //记一下一共有多少个污点对象
//                                int count = 0;
//                                for (CSObj obj : pts2.getObjects()){
//                                    if(obj.getObject().getDeclareClassName() !=null){
//                                        classSet1.add(obj.getObject().getDeclareClassName());
//                                        count++;
//                                    }
//                                }
...
//        });
    ...
    /**
     * 这个方法用于完善隐式污点对象的创建，由于所有的函数调用都有可能涉及子类的扩展
     * 在这里面会对所有的污点对象应用cha追溯其中子类
     * 我们不希望考虑基本类型，当然数组类型也是我们的阻碍
     *
     * 注：该函数的触发方式为某一个对象由于base.invoke,根据多态我们需要这么考虑情况
     * 所以declareClass应该是一致的
     * */

    private PointsToSet dealTaintByCha(CSVar recv, PointsToSet pts){
        //这一步的减枝是为什么？ 本质上就是为了排除那些没有目标声明方法的类，
        //具体来说，我们在生成多态的考虑类时，生成了很多隐式对象，这些对象并不一定都有我们所要调用的方法
        //所以对于我们来说，如果没有方法的去进行下一步操作的话，会造成空间浪费
        Var var = recv.getVar();
        List<JMethod> targetMethods = new ArrayList<>();
        for (Invoke callsite : var.getInvokes()){
            try{
                if(callsite.getContainer().getName().equals("beanHashCode")){
                    System.out.println("hh");
                }
                targetMethods.add(callsite.getRValue().getMethodRef().resolve());
            }catch (Exception e){
                System.out.println("not find method " + callsite.getRValue().getMethodRef());
            }

        }

        TaintManager manager = new TaintManager(this.getHeapModel());
        CSManager csManager = this.getCSManager();
        PointsToSet newTaints = this.makePointsToSet();
        pts.forEach(recvObj ->{
            Obj obj = recvObj.getObject();
            if (manager.isTaint(obj) && !obj.isImplicited() && !MyUtils.list.contains(obj.getType().getName())){
                Collection<Type> allTypes = MyUtils.getAllTypes(obj.getType(), null, targetMethods, false);
                allTypes.forEach(type -> {
                    ...
                    //CSObj cstaint = csManager.getCSObj(this.getContextSelector().getEmptyContext(), taint);
                    newTaints.addObject(cstaint);
                });
            }
            if(!manager.isTaint(obj) && !obj.isImplicited() && !MyUtils.list.contains(obj.getType().getName()) && obj.getType() instanceof ClassType classType && classType.getJClass().isInterface()){
                JClass jInterface = classType.getJClass();
                if (hierarchy.getDirectImplementorsOf(jInterface).size() == 1){
                    for(JClass implementer : hierarchy.getDirectImplementorsOf(jInterface)){
                        Object alloc = obj.getAllocation();
                        Obj newObj = heapModel.getMockObj(Descriptor.ENTRY_DESC,alloc, implementer.getType());
                        CSObj csNewObj = csManager.getCSObj(contextSelector.getEmptyContext(),newObj);
                        newTaints.addObject(csNewObj);
                    }
                }
            }
        });
        pts.addAll(newTaints);
//        this.getPointsToSetOf(recv).addAll(pts);
        return pts;
    }
    ...
    private void processCall(CSVar recv, PointsToSet pts) {
        ...
        //这里没有把cha产生的数据加入到原来pts当中，不知道会不会产生影响，如果
        if (!var.getName().equals("%this")){
            pts = dealTaintByCha(recv, pts);
        }
        recv.getPointsToSet().addAll(pts);

        //pts = dealTaintByCha(recv, pts);
        //the common process
        for (Invoke callSite : var.getInvokes()) {
            pts.forEach(recvObj -> {
//                if(callSite.getContainer().getReturnType().getName().equals("int") && callSite.getContainer().getName().equals("hash")){
//                    System.out.println("find that");
//                }
                //get alltype of taint obj
