...
//        }
    ...
    private void findPaths(CSMethod csMethod, int maxdepth, List<JClass> jClassList, List<String> sourceMethod) {
        ...
        stack.push(new Pair(csMethod, new ArrayList<>(), 0, null,  new ArrayList<>(), new HashSet<>()));
        ...
        while (!stack.isEmpty()) {
            ...
            Set<JMethod> usedMethod = current.usedMethods;
...
//            }
            ...
            if (depth > maxdepth || usedMethod.contains(node.getMethod())){
                continue;
            }

            path.add(node.getMethod());
            usedMethod.add(node.getMethod());
                ...
                for (CSCallSite childcallsite : solver.getCallGraph().getCallersOf(node)) {
                    ...
                    if ((context != null && thisVar != null && hasTainted(context, thisVar)) || dealMethod.getMethod().isStatic() || (context != null && params != null && !params.isEmpty() && judgeParams(context, dealMethod.getMethod().getIR().getParams()))){
                        ...
                        if (!childcallsite.getContainer().getMethod().equals(node.getMethod())){
                            stack.push(new Pair(childcallsite.getContainer(), new ArrayList<>(path), depth + 1, childcallsite, new ArrayList<>(callSites), new HashSet<>(usedMethod)));
                        }
                    }
                }
            }
...
//    }
    ...
    static class Pair {
        ...
        Set<JMethod> usedMethods;


        CSCallSite csCallSite;
        int depth;

        Pair(CSMethod node, List<JMethod> path, int depth, CSCallSite csCallSite, List<CSCallSite> callSites, Set<JMethod> usedMethods) {
            this.node = node;
            this.path = path;
            this.depth = depth;
